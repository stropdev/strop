...
fn counts_multiply_non_operator_commands() {
    ...
    e.feed_text("2x");
    assert_eq!(e.buf().text().to_string(), "cde\n");
    // 2u undoes twice
    let mut e = Editor::new(Buffer::from_text("a\nb\nc\n"));
    e.feed_text("dd");
    e.feed_text("dd");
    assert_eq!(e.buf().text().to_string(), "c\n");
    e.feed_text("2u");
    assert_eq!(e.buf().text().to_string(), "a\nb\nc\n");
    // 3rx replaces three
    let mut e = Editor::new(Buffer::from_text("abcde\n"));
    e.feed_text("3rx");
    assert_eq!(e.buf().text().to_string(), "xxxde\n"); // 3 chars, not 4
                                                       // 2p pastes twice
    let mut e = Editor::new(Buffer::from_text("ab\n"));
    e.feed_text("yiw");
    e.feed_text("2p");
    assert_eq!(e.buf().text().to_string(), "aababb\n"); // register twice at one spot
}
...
fn counted_insert_repeats_text() {
    ...
    assert_eq!(e.buf().text().to_string(), "ZZZx\n"); // vim: i inserts before
...
fn c_enters_insert_at_deletion_start() {
    ...
    e.feed_text("wCthere");
    e.feed(crate::editor::Key::Esc);
    assert_eq!(e.buf().text().to_string(), "hello there\n");
    let mut e = Editor::new(Buffer::from_text("abcdef\n"));
    e.feed_text("3lCX");
    e.feed(crate::editor::Key::Esc);
    assert_eq!(e.buf().text().to_string(), "abcX\n");
}
