...
fn fixture() -> Editor {
    ...
    editor.frame_draw = Some(test_frame_draw);
...
/// The engine's test renderer (0046): deterministic plain-text cells via a
/// TestBackend, with the same observe/check edge the binary's draw has.
/// Cell-grid production itself stays in the binary.
fn test_frame_draw(editor: &mut Editor, columns: u16, rows: u16, _: bool) -> io::Result<()> {
    let mut terminal = ratatui::Terminal::new(ratatui::backend::TestBackend::new(columns, rows))?;
    terminal.draw(|frame| {
        let area = frame.area();
        for (row, line) in editor.buf().text().lines().enumerate() {
    ...
    if editor.tape.observes() {
        ...
        let cells: Vec<String> = (0..rows)
            .map(|y| (0..columns).map(|x| buffer[(x, y)].symbol()).collect())
            .collect();
        editor
            .tape
            .check(&serde_json::json!({"columns": columns, "rows": rows, "cells": cells}))?;
    }
    Ok(())
}
...
fn replay_fixture(editor: &Editor) -> Editor {
    ...
    drive::replay(editor.tape.fixture_nodes(), test_frame_draw).unwrap()
}
...
fn changing_recorded_request_identity_fails_before_native_launch() {
    ...
    assert!(drive::replay(nodes, test_frame_draw).is_err());
}
