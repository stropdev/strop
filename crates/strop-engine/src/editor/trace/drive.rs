...
impl Editor {
        ...
        match action {
            ...
            Action::Frame { columns, rows } => {
                ...
                if !self.should_quit && !self.docs.is_empty() {
                    // Renders through the injected frame renderer: in
                    // replay that draw consumes the recorded cell
                    // observation. The binary installs the hook at its
                    // composition roots (0046).
                    let draw = self
                        .frame_draw
                        .ok_or_else(|| io::Error::other("frame draw hook not installed"))?;
                    draw(self, columns, rows, false)?;
                }
            }
            Action::Finish => {
    ...
    fn observation(&self) -> serde_json::Value {
        ...
        serde_json::json!({
            ...
            "headless": crate::editor::state_json(self),
...
/// The frame renderer replay checks recorded cells against; installed by
/// the caller since cell-grid production lives outside the engine.
pub fn replay(
    nodes: Vec<strop_trace::replay::Node>,
    frame_draw: crate::editor::FrameDraw,
) -> io::Result<Editor> {
    ...
    editor.frame_draw = Some(frame_draw);
