---- MODULE SearchLifecycle ----
(***************************************************************************)
(* The canonical search lifecycle (0063 §6.4/§6.5) — one focused model in  *)
(* the established registry, at the publication boundary the Rust code     *)
(* actually has: generation-tagged jobs, exact admission, evidence that    *)
(* arrives late, bounded warm-up, honest completion.                       *)
(*                                                                       *)
(*   1. every provider job (rg batch, symbol index, catalog scan, LSP      *)
(*      workspace query) carries the query generation that armed it;       *)
(*      publication is guarded on that generation being current            *)
(*      (Rust: ticket ownership in handle_picker_event, the wsymbols      *)
(*      generation check in merge_workspace_symbols, trace-rejected        *)
(*      otherwise)                                                        *)
(*   2. a new query retires the old generation AND purges its rows —       *)
(*      a retired query can never publish, not even late                   *)
(*   3. rows carry the source revision they were extracted against;       *)
(*      acceptance re-checks it (Rust: dirty buffers overlay their disk    *)
(*      entry before admission — SymbolIndex::overlay)                     *)
(*   4. completion is honest: the "no more coming" flag requires the       *)
(*      current generation's terminal event and no truncation; bounds     *)
(*      hits stay visibly incomplete (Rust: coverage_gap → Warning,        *)
(*      never a silent empty success — R9)                                *)
(*   5. warm-up is bounded: at most WARM_MAX servers in flight, started   *)
(*      only while the symbols surface is open (Rust: lsp_drain_warm_queue)*)
(*                                                                       *)
(* THE invariants (§6.5's named safety properties):                       *)
(*                                                                       *)
(*   RowsCurrent            no retired query's rows exist                  *)
(*                          (no retired-query publication)                 *)
(*   RowsInScope            every row belongs to the opened scope          *)
(*                          (no foreign results)                          *)
(*   StaleAcceptsNever      acceptance never consumed a row whose source   *)
(*                          revision moved (no known-stale symbol          *)
(*                          acceptance)                                   *)
(*   CompletionHonest       the complete flag implies the current          *)
(*                          generation's terminal, untruncated            *)
(*                          (no false completeness)                       *)
(*   WarmBounded            warm-up concurrency within the bound          *)
(*                                                                       *)
(* Not modeled: text content and scoring (the Rust oracles own them —     *)
(* the differential reference interpreter in query/properties.rs),        *)
(* LSP configuration scoping per project (attach keys), liveness.         *)
(*                                                                       *)
(* The kept mutant (SearchLifecycle_Mutant.tla) drops the generation       *)
(* guard on publication and the revision guard on acceptance; it must     *)
(* die by exactly RowsCurrent + StaleAcceptsNever.                        *)
(***************************************************************************)
